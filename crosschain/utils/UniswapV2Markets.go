...
import (
	...
	UniswapQuery "crosschain/UniswapQuery"
	UniswapV2Factory "crosschain/generatedContracts"
...
func (uniswapMarkets *UniswapV2Markets) UpdateMarkets(
	...
	for i := 0; i < len(allMarkets); i++ {
		for j := 0; j < len(allMarkets[i]); j++ {
			...
			if In(allMarkets[i][j].Token0Address.String(), tokensOfInterest) {
...
func (uniswapMarkets *UniswapV2Markets) EvaluateCrossMarkets(tokensOfInterest []Token) {
	...
	for _, token := range tokensOfInterest {
		...
		for tokenAddress, market := range uniswapMarkets.Asset[token.Symbol][token.Protocol].CrossMarketsByToken {
			...
			if priceDiff0.Cmp(priceDiff1) == 1 && priceDiff0.Cmp(big.NewFloat(1.3)) == -1 {
				priceDiff0 = priceDiff0.Sub(priceDiff0, big.NewFloat(0.2))
				if priceDiff0.Cmp(big.NewFloat(1)) == 1 {
					market.CurrentArbitrageOpp = priceDiff0
				}
			} else if priceDiff1.Cmp(priceDiff0) == 1 && priceDiff1.Cmp(big.NewFloat(1.3)) == -1 {
				priceDiff1 = priceDiff1.Sub(priceDiff1, big.NewFloat(0.2))
				if priceDiff1.Cmp(big.NewFloat(1)) == 1 {
