...
const MTIC_ETHEREUM = "0x7D1AfA7B718fb893dB30A3aBc0Cfc608AaCfeBB0"
const MTIC_POLYGON = "0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270"

func initSet(name string) *TokenSet {
	return &TokenSet{
    name: name,
...
func (ts *TokenSet) add(token string) error {
	_, exists := ts.set[token]
	if exists {
		return fmt.Errorf("ERROR: Add(int64)")
	}
	ts.set[token] = struct{}{}
	return nil
}
...
func (ts *TokenSet) remove(token string) error {
	_, exists := ts.set[token]
	if !exists {
		return fmt.Errorf("ERROR: Remove(int64)")
	}
	delete(ts.set, token)
	return nil
}
...
func InitSets() {
  ...
  MTIC = initSet("MTIC")
  MTIC.add(MTIC_ETHEREUM)
  MTIC.add(MTIC_POLYGON)
  sets[7] = MTIC
}

func FetchSet(token string) []string {
	for i := 0; i < tokens; i++ {
		if !sets[i].exists(token) {
			continue
		}
...
func FetchType(token string) *string {
  for i := 0; i < tokens; i++ {
		if !sets[i].exists(token) {
			continue
		}
