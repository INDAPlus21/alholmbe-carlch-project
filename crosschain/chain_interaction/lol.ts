...
import { WBNB_ADDRESS as WETH_ADDRESS } from './addresses';
...
export function getBestCrossedMarket(
  ...
  // we have some crossed markets, and need to determine which one is the best
  ...
  for (const crossedMarket of crossedMarkets) {
    ...
    for (const size of TEST_VOLUMES) {
      // for this size, how much would I make from buying on market x and selling market y?
      // buy
      ...
      // see if we can find something even better
      if (
...
export class Arbitrage {
  ...
  async evaluateMarkets(
    ...
    // global bestCrossedMarkets from all our pairs
    const bestCrossedMarkets = new Array<CrossedMarketDetails>();

    for (const tokenAddress in marketsByToken) {
      // a pair has 1+ markets
      ...
      // we now know what we can buy/sell the tokens for

      const crossedMarkets = new Array<Array<EthMarket>>();
      for (const pricedMarket of pricedMarkets) {
        // if sell price at market x > buy price at market y
        // then push both markets to crossedMarkets
        _.forEach(pricedMarkets, (pm) => {
          // can we sell for more than we buy?
      ...
      // we now know which markets have profitable opportunities
      const bestCrossedMarket = getBestCrossedMarket(
        crossedMarkets,
        tokenAddress
      );

      // if there is no crossedMarket, bestCrossedMarket will be undefined
      if (
        bestCrossedMarket !== undefined &&
    ...
    // sort the crossed markets by profit
